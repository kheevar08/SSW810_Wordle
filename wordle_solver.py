...
def wordle_solver():
    ...
    while(attempts<7):              # user has 6 attempts to guess the correct word
        temp = wordle.play_wordle_solver(answer,attempts,word)
        if temp == 0:
            break
        if temp != 0:
            temp_list = ['','','','','']
            ...
            if temp_list[0] == '"' and not badLetters.__contains__(word[0]):
                badLetters.append(word[0])
            if temp_list[1] == '"' and not badLetters.__contains__(word[1]):
                badLetters.append(word[1])
            if temp_list[2] == '"' and not badLetters.__contains__(word[2]):
                badLetters.append(word[2])
            if temp_list[3] == '"' and not badLetters.__contains__(word[3]):
                badLetters.append(word[3])
            if temp_list[4] == '"' and not badLetters.__contains__(word[4]):
